...
Examples:
  ...
  qumulo_file_filter_mac.sh --path /home --older-than 90 --larger-than 1GB --smaller-than 10GB --csv-out results.csv
...
# BSD date uses -v flag: -v-Nd means "N days ago"
threshold_older=""
threshold_newer=""
comparison=""
...
if [ -n "$OLDER_THAN" ] && [ -n "$NEWER_THAN" ]; then
    ...
    threshold_older=$(date -u -v-${OLDER_THAN}d +%s)
    threshold_newer=$(date -u -v-${NEWER_THAN}d +%s)
    comparison="range"
...
if [ -n "$ACCESSED_OLDER_THAN" ]; then
    ACCESSED_THRESHOLD_OLDER=$(date -u -v-${ACCESSED_OLDER_THAN}d +%s)
fi
if [ -n "$ACCESSED_NEWER_THAN" ]; then
    ACCESSED_THRESHOLD_NEWER=$(date -u -v-${ACCESSED_NEWER_THAN}d +%s)
fi
if [ -n "$MODIFIED_OLDER_THAN" ]; then
    MODIFIED_THRESHOLD_OLDER=$(date -u -v-${MODIFIED_OLDER_THAN}d +%s)
fi
if [ -n "$MODIFIED_NEWER_THAN" ]; then
    MODIFIED_THRESHOLD_NEWER=$(date -u -v-${MODIFIED_NEWER_THAN}d +%s)
fi
if [ -n "$CREATED_OLDER_THAN" ]; then
    CREATED_THRESHOLD_OLDER=$(date -u -v-${CREATED_OLDER_THAN}d +%s)
fi
if [ -n "$CREATED_NEWER_THAN" ]; then
    CREATED_THRESHOLD_NEWER=$(date -u -v-${CREATED_NEWER_THAN}d +%s)
fi
if [ -n "$CHANGED_OLDER_THAN" ]; then
    CHANGED_THRESHOLD_OLDER=$(date -u -v-${CHANGED_OLDER_THAN}d +%s)
fi
if [ -n "$CHANGED_NEWER_THAN" ]; then
    CHANGED_THRESHOLD_NEWER=$(date -u -v-${CHANGED_NEWER_THAN}d +%s)
fi
...
fi | python3 -u -Wignore::DeprecationWarning -c "
import sys
import json
import csv
import time
